...
def analyze_news_impact_with_gemini(news_list):
    """Gemini API 호출 및 자세한 에러 리포트"""
    # 안정성이 가장 높은 gemini-1.5-flash 모델 사용
...
위 뉴스를 바탕으로 국내 증시(삼성전자, SK하이닉스, KODEX 200 등)에 미칠 영향과 전망을 3줄로 요약/예측해 주세요.
"""
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # 에러 발생 시 원인을 메시지에 직접 표시
            error_msg = res.json().get('error', {}).get('message', res.text)
            return f"⚠️ API 에러 발생 (코드 {res.status_code}):\n{error_msg}"
